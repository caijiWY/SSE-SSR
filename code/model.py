...
class G_NET(nn.Module):
    def __init__(self):
        ...
        #----------new add SE module--------------
        self.se_64 = SEBlock(cfg.GAN.GF_DIM, cfg.GAN.GF_DIM)
        self.se_128 = SEBlock(cfg.GAN.GF_DIM, cfg.GAN.GF_DIM)
        #
    ...
    def forward(self, z_code, sent_emb, word_embs, mask, cap_lens):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            ...
            # add SE module
            feat_64 = self.se_64(h_code1, h_code2)
            #
            fake_img2 = self.img_net2(feat_64)
        ...
        if cfg.TREE.BRANCH_NUM > 2:
            ...
            # add SE module
            feat_128 = self.se_128(feat_64, h_code3)
            fake_img3 = self.img_net3(feat_128)
            #
...
class SEBlock(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.main = nn.Sequential(  nn.AdaptiveAvgPool2d(4),  # 4 * 4
...
class D_NET64(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        layers = []
        # layers.append(nn.Conv2d(ndf * 8, ndf * 8, kernel_size=4, padding=1, stride=2, bias=False))
        # layers.append(nn.LeakyReLU(0.01))
        # layers.append(nn.Conv2d(ndf * 8, ndf * 8, kernel_size=4, padding=1, stride=2, bias=False))
        # layers.append(nn.LeakyReLU(0.01))
        layers.append(nn.Conv2d(ndf * 8, 40, kernel_size=4, padding=0, stride=1, bias=False))
        layers.append(nn.LeakyReLU(0.01))
        self.conv2 = nn.Sequential(*layers)

    def forward(self, x_var):
        x_code4 = self.img_code_s16(x_var)  # 4 x 4 x 8df
        out_cls = self.conv2(x_code4)
        return [x_code4, out_cls.view(out_cls.size(0), out_cls.size(1))]
...
class D_NET128(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        self.COND_DNET = D_GET_LOGITS(ndf, nef, bcondition=True)
        layers = []
        # layers.append(nn.Conv2d(ndf * 16, ndf * 16, kernel_size=4, padding=1, stride=2, bias=False))
        # layers.append(nn.LeakyReLU(0.01))
        layers.append(nn.Conv2d(ndf * 16, 40, kernel_size=4, padding=0, stride=1, bias=False))
        layers.append(nn.LeakyReLU(0.01))
        self.conv2 = nn.Sequential(*layers)
...
class D_NET256(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        #-------------------------------
        layers = []
        layers.append(nn.Conv2d(ndf * 32, 40, kernel_size=4, padding=0, stride=1, bias=False))
        layers.append(nn.LeakyReLU(0.01))
        self.conv2 = nn.Sequential(*layers)
        ...
        self.rf_big = nn.Sequential(
            conv2d(nfc[8], nfc[8], 1, 1, 0, bias=False),
    ...
    def forward(self, x_var, st, part = None):#new
        if type(x_var) is not list:
            imgs = [F.interpolate(x_var, size=256), F.interpolate(x_var, size=128)]#new
        x_code16 = self.img_code_s16(x_var)
        feat_32 = x_code16#new
        x_code8 = self.img_code_s32(x_code16)
        feat_last = x_code8#new
        x_code4 = self.img_code_s64(x_code8)
        out_cls = self.conv2(x_code4)#new
        ...
        if st == 'real':
            ...
            if part==3:
                rec_img_part = self.decoder_part(feat_32[:,:,8:,8:])
            return [x_code4, out_cls.view(out_cls.size(0), out_cls.size(1)), torch.cat([rf_0, rf_1]), [rec_img_big, rec_img_small, rec_img_part]]
        return [x_code4, out_cls.view(out_cls.size(0), out_cls.size(1)), torch.cat([rf_0, rf_1])]
