...
class condGANTrainer(object):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                for i in range(len(netsD)):
                    netsD[i].zero_grad()
                    if i == 2:
                        errD, err_real_stage3, err_fake_stage3 = discriminator_loss(netsD[i], imgs[i], fake_imgs[i],
                                                sent_emb, real_labels, fake_labels,i)
                        # backward and update parameters
                        errD.backward()
                        optimizersD[i].step()
                        errD_total += errD
                        D_logs += 'err_real_stage3: %.2f err_fake_stage3: %.2f ' % (err_real_stage3, err_fake_stage3)
                    else:
            ...
            print('-' * 89)
            file=open('/root/DM-GAN-master/output/time.txt','a')
            file.write('%.2f\n'%(end_t - start_t))
            file.close()
    ...
    def sampling(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            split_dir1 = 'original'
            s_tmp = model_dir[:model_dir.rfind('.pth')]
            save_dir1 = '%s/%s' % (s_tmp, split_dir1)
            mkdir_p(save_dir1)
